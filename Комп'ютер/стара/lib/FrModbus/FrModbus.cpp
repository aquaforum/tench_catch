...
FrModbus::FrModbus(): nPort(502) ,nC(0) // кноструктор, треба уточнити
{
    linkOk=false;
...
FrModbus::~FrModbus() // поки-що тривіальний деструктор
{
    pS->close();
}

void FrModbus::slotConnected () // приєдналися
{
    //connSend->start();
    connTimeout->start();
    nLen=0;
...
void FrModbus::slotNewConnect()
{
    connWait->stop();
    pS->connectToHost(sHostname,nPort);
}

void FrModbus::slotTimeout() // таймаут отримання даних від сервера
{
//   connSend->stop();
    connTimeout->stop();
    connWait->start();
...
void FrModbus::slotDisconnect() // відєднання зі сторони сервера
{
    //connSend->stop(); // зупинити таймер, коли від’єднано немає сенсу слати запити
    pS->close();
}

void FrModbus::slotError(QAbstractSocket::SocketError)
{
    //connSend->stop();
    connTimeout->stop();
    connWait->start();
...
int FrModbus::loadList(QString fileName)
{
...
void FrModbus::setHostName(QString hostName)
{
    sHostname=hostName;
}

void FrModbus::setPort(int Port)
{
    nPort=Port;
}
...
void FrModbus::sendValue(QString tag,qint16 v)
{
   QByteArray q;
...
void FrModbus::sendValue(QString tag,qint32 v)
{
    QVector<qint16> t(2);
    *((qint32*)t.data())=v;
...
void FrModbus::sendValue(QString tag,double v)
{
    QVector<qint16> t(2);
...
void FrModbus::sendValue(QString tag,QVector<qint16> &v)
{
   QByteArray q;
   QDataStream qry(&q,QIODevice::WriteOnly);
