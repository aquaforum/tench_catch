...
#if QT_VERSION_MAJOR>4
    QRect pos=QRect((QApplication::desktop()->screenGeometry(-1).width()-w)/2,(QApplication::desktop()->screenGeometry(-1).height()-h)/2,w,h);
#else
...
void RpanelReg::changeReg(int Index) // зміна регулятор
{
    ...
    ui->Value_1->setText(src.getDescription(RegDes[RegNum][Ri::PV_1].split('.')[0]));
    ...
    QString PV_1=RegDes[RegNum][Ri::PV_1].split('.').at(0);

    ui->scalePV_1->setScaleMinMax(src.scaleZero(PV_1),src.scaleFull(PV_1));


    if(RegDes[RegNum][Ri::PV_1].split('.').size()>1)
    {
        ui->sbSP_1->setDecimals(1);
        ui->sbSP_1->setSingleStep(0.1);
    }
    else
    {
        if(fabs(src.scaleFull(PV_1))<20)
        {
...
//     // SP_1
    ...
    QString SP_1=RegDes[RegNum][Ri::SP_1].split('.').at(0);

    if(src.scaleZero(SP_1)<src.scaleFull(SP_1))
    {
        ui->sbSP_1->setMinimum(src.scaleZero(SP_1));
        ui->sbSP_1->setMaximum(src.scaleFull(SP_1));

    }
    else
    {
        ui->sbSP_1->setMaximum(src.scaleZero(SP_1));
        ui->sbSP_1->setMinimum(src.scaleFull(SP_1));
    }

    ui->sbSP_1->setValue(src.getValueScaled(SP_1));
    ui->sbSP_1->blockSignals(false);

    ui->vsSP_1->blockSignals(true);
    ui->vsSP_1->setValue(src.getValueFloat(SP_1));
    ...
    if(src.getTags().contains(SP_2))
    {
        ...
        ui->sbSP_2->setValue(src.getValueScaled(SP_2));
...
void RpanelReg::updateTrend(int len)
{
    ...
    QString fields=RegDes[RegNum][Ri::PV_1].split('.')[0];
...
void RpanelReg::updateData() // поновлення даних у віджетах
{
    ...
    if(RegDes[RegNum][Ri::PV_1].split('.').size()>1)
    {
        ui->lePV_1->setText(QString("%1").arg(src.getValueScaled(PV_1),4,'f',1));
    }
    else
    {
        if(fabs(src.scaleFull(PV_1)<50.0 && src.scaleFull(PV_1))>20.0)
            ui->lePV_1->setText(QString("%1").arg(src.getValueScaled(PV_1),4,'f',1));
        else if (fabs(src.scaleFull(PV_1))<=20.0)
            ui->lePV_1->setText(QString("%1").arg(src.getValueScaled(PV_1),4,'f',2));
        else
           ui->lePV_1->setText(QString("%1").arg(src.getValueScaled(PV_1),3,'f',0));

    }

    int t1 = src.getValueFloat(PV_1);
    if(t1<0) t1=0;
    if(t1>4000) t1=4000;

    ui->pbPV_1->setValue(t1);

    v << src.getValueFloat(PV_1);
  ...
  { // якщо не знайдено
    ...
    switch( ix )
    {
        ...
        case Ri::SP_1: // sbSP_1
            ...
            ui->vsSP_1->blockSignals(true);
            ui->vsSP_1->setValue((v-src.scaleZero(RegDes[RegNum][ix].split('.')[0]))/
                    (src.scaleFull(RegDes[RegNum][ix].split('.')[0]) - src.scaleZero(RegDes[RegNum][ix].split('.')[0])) *4000.0);
            ui->vsSP_1->blockSignals(false);
    ...
    //qDebug() << "Tag" << RegDes[RegNum][ix];
      switch( ix )
      {
        case Ri::SP_1: // vsSP_1
            ...
            ui->sbSP_1->blockSignals(true);
            ui->sbSP_1->setValue(((double)v/4000.0*(src.scaleFull(RegDes[RegNum][ix].split('.')[0])
                                  -src.scaleZero(RegDes[RegNum][ix].split('.')[0])) + src.scaleZero(RegDes[RegNum][ix].split('.')[0]) ));
            ui->sbSP_1->blockSignals(false);
	    break;
        case Ri::SP_2: // vsSP_2
            src.sendValue(RegDes[RegNum][ix].split('.')[0],(double)v);
            ...
            ui->sbSP_2->blockSignals(true);
            ui->sbSP_2->setValue(((double)v/4000.0*(src.scaleFull(RegDes[RegNum][ix].split('.')[0])-src.scaleZero(RegDes[RegNum][ix].split('.')[0])) + src.scaleZero(RegDes[RegNum][ix].split('.')[0]) ));
            ui->sbSP_2->blockSignals(false);
	    break;
        case Ri::SP_3: // vsSP_3
