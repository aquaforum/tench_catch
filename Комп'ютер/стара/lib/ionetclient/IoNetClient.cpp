...
void IoNetClient::slotSendQuery()
{
    ...
    for(int i=0;i<src.size();++i)  // перебрати по кількості джерел даних
    {
          ...
          QFile BoolLink(QDir::homePath() + "/isLincked");
          if(BoolLink.exists())
          {
//              qDebug()<<"File here";
              qry << qint8('R') << qint8('L') << qint8(i) << qint16(0) << qint16(0); // сформувати запит на отримання списку тегів
          }
          //else
          //  qDebug()<<"No need link OK";
