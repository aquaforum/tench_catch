...
RxModbus::RxModbus(): nPort(502) ,nC(0) ,unitId(1)// кноструктор, треба уточнити
{
    linkOk=false;
        ...
        for(i=0;!f.atEnd();++i)
        {
            ...
            if(sl.size()>4) // якщо є всі поля
            {
                ...
                if(packet_len>124 || current_addr>next_addr || current_rf!=last_rf || (current_ft==5 && last_ft!=5) || (current_ft!=5 && last_ft==5)) //виявити дірки, межі пакунків, кратність читання чи зміну типу
                {
                    ...
                    packet_len=current_len;
                    qry << qint16(wc_last) << qint16(0) << qint16(6) << unitId <<  qint8(sl[2]=="EBOOL"?GETMCR:GETMHR) << qint16(current_addr-1); // ід транзакції << ід протокола << довжина << адреса слейва << код функції << стартова адреса
