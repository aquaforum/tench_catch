...
RxModbusS::RxModbusS(): nPort(502) ,nC(0) // кноструктор, треба уточнити
{
    linkOk=false;
...
RxModbusS::~RxModbusS() // поки-що тривіальний деструктор
{
    pS->close();
}

void RxModbusS::slotConnected () // приєдналися
{
    //connSend->start();
    connTimeout->start();
    nLen=0;
...
void RxModbusS::slotNewConnect()
{
    connWait->stop();
    pS->connectToHost(sHostname,nPort);
}

void RxModbusS::slotTimeout() // таймаут отримання даних від сервера
{
//   connSend->stop();
    connTimeout->stop();
    connWait->start();
...
void RxModbusS::slotDisconnect() // відєднання зі сторони сервера
{
    //connSend->stop(); // зупинити таймер, коли від’єднано немає сенсу слати запити
    pS->close();
}

void RxModbusS::slotError(QAbstractSocket::SocketError)
{
    //connSend->stop();
    connTimeout->stop();
    connWait->start();
...
int RxModbusS::loadList(QString fileName)
{
...
void RxModbusS::setHostName(QString hostName)
{
    sHostname=hostName;
}

void RxModbusS::setPort(int Port)
{
    nPort=Port;
}

void RxModbusS::start()
{
...
/*
void RxModbusS::sendValue(QString tag,qint16 v)
{
   QByteArray q;
...
void RxModbusS::sendValue(QString tag,qint32 v)
{
    QVector<qint16> t(2);
    *((qint32*)t.data())=v;
...
void RxModbusS::sendValue(QString tag,double v)
{
    QVector<qint16> t(2);
...
void RxModbusS::sendValue(QString tag,QVector<qint16> &v)
{
   QByteArray q;
   QDataStream qry(&q,QIODevice::WriteOnly);
