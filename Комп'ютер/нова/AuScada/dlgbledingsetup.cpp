...
dlgBledingSetup::dlgBledingSetup(AuBase &source,QWidget *parent) :
    QDialog(parent),
    ...
    src(source)
{
    ...
    foreach(QTimeEdit* v,te_m)
    {
        ...
        v->setTime(t.addMSecs(src[SATUR][v->objectName()].valueReal()*60000)); // 000
//        qDebug() << v->objectName() << t.addMSecs(SATUR[0]->getValue16(v->objectName())*60000);
    ...
    {
        t.setHMS(0,0,0,0);
        v->setTime(t.addMSecs(src[SATUR][v->objectName()]*1000)); //
        connect(v,SIGNAL(timeChanged(QTime)),this,SLOT(slotSet(QTime)));
    ...
    {
        v->setCurrentIndex(src[SATUR][v->objectName()]!=0?1:0);
    ...
    {
        v->setCurrentIndex(s.contains(src[SATUR][v->objectName()])?s[src[SATUR][v->objectName()]]:0);
        connect(v,SIGNAL(currentIndexChanged(int)),this,SLOT(slotSet(int)));
    }
    ...
    src[SATUR][QStringLiteral("yy_s")].setValues(time);
    updateList();
    ...
    ui->d_en_p_01->setCurrentIndex(src[DIFUZ][QStringLiteral("en_p_01")]!=0?1:0);
    connect(ui->d_en_p_01,SIGNAL(currentIndexChanged(int)),this,SLOT(d_slotSet(int)));

    t.setHMS(0,0,0,0);
    ui->d_Tp_zd_01->setTime(t.addMSecs(src[DIFUZ][QStringLiteral("Tp_zd_01")]*1000)); // *1000
    connect(ui->d_Tp_zd_01,SIGNAL(timeChanged(QTime)),this,SLOT(d_slotSet(QTime)));

    t.setHMS(0,0,0,0);
    ui->d_Tm_zd_01->setTime(t.addMSecs(src[DIFUZ][QStringLiteral("Tm_zd_01")]*60000)); // 000
    connect(ui->d_Tm_zd_01,SIGNAL(timeChanged(QTime)),this,SLOT(d_slotSet(QTime)));

    ui->d_kr_p_01->setCurrentIndex(s.contains(src[DIFUZ][QStringLiteral("kr_p_01")])?s[src[DIFUZ][QStringLiteral("kr_p_01")]]:0);
...
void dlgBledingSetup::slotSet(int v)
{
    ...
    {
        src[SATUR][sender()->objectName()]=qint16(-v);
    }
    else
    {
        QVector<qint16> vi;
        vi << 1440 << 720 << 480 << 360 << 240  << 180 << 120 << 60 << 30;
        src[SATUR][sender()->objectName()]=vi[v];
    }
...
void dlgBledingSetup::slotSet(QTime v)
{
    ...
    if(sender()->objectName().left(2)=="Tm")
        t= QTime(0,0,0,0).msecsTo(v)/60;
    else
        t= QTime(0,0,0,0).msecsTo(v)/1000; // /1000;

    src[SATUR][sender()->objectName()]=t;
    //SATUR[0]->sendValue("Save",qint16(-1));
...
void dlgBledingSetup::d_slotSet(QTime v)
{
    ...
    qint16 t=0;
    if(tag=="Tm_zd_01")
        t= QTime(0,0,0,0).msecsTo(v)/60; // 000;
    else
        t= QTime(0,0,0,0).msecsTo(v)/1000; // /1000;
